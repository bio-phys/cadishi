...
#pragma omp atomic write
         ...
         for (int i=0; i<j; ++i) {
            d[i] = (int) (scal * dist<TUPLE3_T, FLOAT_T, box_type_id>(p[i], p[j], box, box_ortho, box_inv, box_half));
         }
         /**
          * Checks of the previously calculated integer-converted distances.
          *
...
#pragma omp for OMP_SCHEDULE
      for (int j=0; j<nelem1; ++j) {
         ...
         for (int i=0; i<nelem2; i+=inner_loop_blocksize) {
            ...
            for (int k=0; k<block_n_elem; ++k) {
               d[k] = (int) (scal * dist<TUPLE3_T, FLOAT_T, box_type_id>(p2[i+k], p1[j], box, box_ortho, box_inv, box_half));
            }
            /**
             * Checks of the previously calculated integer-converted distances.
             *
...
#pragma omp atomic write
         ...
         for (int i=0; i<nelem2; ++i) {
            d[i] = (int) (scal * dist<TUPLE3_T, FLOAT_T, box_type_id>(p2[i], p1[j], box, box_ortho, box_inv, box_half));
         }
         /**
          * Checks of the previously calculated integer-converted distances.
          *
         ...
         switch (box_type_id) {
            case none:
               distances_ptr[idx] = dist <TUPLE3_T, FLOAT_T, none>
                                       (r_copy[j], r_copy[i], box_copy, box_ortho, box_inv, box_half);
               break;
            case orthorhombic:
